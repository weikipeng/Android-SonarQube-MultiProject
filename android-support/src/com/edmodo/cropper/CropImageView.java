...
import com.eduu.bang.android_support.R;
...
public class CropImageView extends FrameLayout {
    ...
    /**
     * add by JGeekDev
     * Gets the without cropped image (maybe rotated)
     *
     * @return the Bitmap representing the without cropped image
     */
    public Bitmap getWithoutCropImageView() {
        return mBitmap;
    }
     ...
     * @return
     */
    public boolean CropSizeFitBmp() {
        Rect imageRect = ImageViewUtil.getBitmapRectCenterInside(mBitmap, mImageView);
        if (Edge.LEFT.getCoordinate() == imageRect.left && Edge.RIGHT.getCoordinate() == imageRect.right
                && Edge.TOP.getCoordinate() == imageRect.top && Edge.BOTTOM.getCoordinate() == imageRect.bottom) {
            return true;
        } else {
            return false;
        }
    ...
    /**
     * add by JGeekDev
     * Sets the crop frame hide or not.
     * 
     * @param isHide Boolean that crop frame show status.
     */
    public void setAspectHide(boolean isHide) {
        if (isHide) {
            mCropOverlayView.setVisibility(GONE);
        } else {
